...
FFT::FFT(){
	...
	std::fill(m_input, m_input + TWO_FRAME_SIZE, 0.f);
	std::fill(m_output, m_output + TWO_FRAME_SIZE, 0.f);
...
void FFT::analyze(float input[FRAME_SIZE], float output[TWO_FRAME_SIZE]){
	std::copy(m_input_prev, m_input_prev + FRAME_SIZE, m_input);
	std::copy(input, input + FRAME_SIZE, m_input + FRAME_SIZE);
	std::copy(input, input + FRAME_SIZE, m_input_prev);
	for (int i = 0; i < TWO_FRAME_SIZE; ++i){
		m_input[i] *= m_ha[i];
	}
	fft(m_input, output, TWO_FRAME_SIZE);
}

void FFT::synthesize(float input[TWO_FRAME_SIZE], float output[FRAME_SIZE]){
	ifft(input, m_output, TWO_FRAME_SIZE);
	for (int i = 0; i < TWO_FRAME_SIZE; ++i){
		m_output[i] *= m_ha[i];
	}
...
Qin Li, Feb 18, 2005
***************************************************************************/
void FFT::fft(float* xin, float* xout, unsigned int n){
	...
	float* tempbuf = new float[2 * n];
	float* sin_tab = new float[n / 4 + 1];
	unsigned int FFTSize = n;
...
Qin Li, Feb 18, 2005
***************************************************************************/
void FFT::ifft(float* xin, float* xout, unsigned int n) {
